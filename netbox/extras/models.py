    ...
    def get_custom_fields(self):
        ...
        if hasattr(self, "pk"):
            values = CustomFieldValue.objects.filter(
                obj_type=content_type, obj_id=self.pk
...
class CustomField(models.Model):
    ...
    type = models.PositiveSmallIntegerField(
        choices=CUSTOMFIELD_TYPE_CHOICES, default=CF_TYPE_TEXT
    )
...
class CustomFieldChoice(models.Model):
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        CustomFieldValue.objects.filter(
            field__type=CF_TYPE_SELECT, serialized_value=str(pk)
        ).delete()
...
class TopologyMap(models.Model):
    ...
    type = models.PositiveSmallIntegerField(
        choices=TOPOLOGYMAP_TYPE_CHOICES, default=TOPOLOGYMAP_TYPE_NETWORK
    )
    ...
    def render(self, img_format="png"):
        ...
        for i, device_set in enumerate(self.device_sets):
            ...
            for query in device_set.strip(";").split(
                ...
                devices += Device.objects.filter(name__regex=query).select_related(
                    "device_role"
                )
    ...
    def add_network_connections(self, devices):
        ...
        for interface in connected_interfaces:
            style = (
                "solid"
                if interface.connection_status == CONNECTION_STATUS_CONNECTED
                else "dashed"
            )
            self.graph.edge(
        ...
        for termination in CircuitTermination.objects.filter(
            term_side="A", connected_endpoint__device__in=devices
        ):
            peer_termination = termination.get_peer_termination()
            if (
                peer_termination is not None
                and peer_termination.interface is not None
                and peer_termination.interface.device in devices
            ):
                self.graph.edge(
                    termination.interface.device.name,
                    peer_termination.interface.device.name,
                    color="blue",
                )
    ...
    def add_console_connections(self, devices):
        ...
        for cp in ConsolePort.objects.filter(
            device__in=devices, connected_endpoint__device__in=devices
        ):
            style = (
                "solid"
                if cp.connection_status == CONNECTION_STATUS_CONNECTED
                else "dashed"
            )
            self.graph.edge(
                cp.connected_endpoint.device.name, cp.device.name, style=style
            )
    ...
    def add_power_connections(self, devices):
        ...
        for pp in PowerPort.objects.filter(
            device__in=devices, connected_endpoint__device__in=devices
        ):
            style = (
                "solid"
                if pp.connection_status == CONNECTION_STATUS_CONNECTED
                else "dashed"
            )
            self.graph.edge(
                pp.connected_endpoint.device.name, pp.device.name, style=style
            )
...
class ConfigContextModel(models.Model):
    ...
    local_context_data = JSONField(blank=True, null=True)
...
class ObjectChange(models.Model):
    ...
    action = models.PositiveSmallIntegerField(choices=OBJECTCHANGE_ACTION_CHOICES)
