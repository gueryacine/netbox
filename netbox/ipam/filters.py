...
class AggregateFilter(CustomFieldFilterSet, django_filters.FilterSet):
    ...
    rir_id = django_filters.ModelMultipleChoiceFilter(
        queryset=RIR.objects.all(), label="RIR (ID)"
    )
...
class PrefixFilter(CustomFieldFilterSet, django_filters.FilterSet):
    ...
    within = django_filters.CharFilter(method="search_within", label="Within prefix")
    within_include = django_filters.CharFilter(
        method="search_within_include", label="Within and including prefix"
    )
    contains = django_filters.CharFilter(
        method="search_contains", label="Prefixes which contain this prefix or IP"
    )
    mask_length = django_filters.NumberFilter(
        method="filter_mask_length", label="Mask length"
    )
    vrf_id = django_filters.ModelMultipleChoiceFilter(
        queryset=VRF.objects.all(), label="VRF"
    )
    ...
    vlan_id = django_filters.ModelMultipleChoiceFilter(
        queryset=VLAN.objects.all(), label="VLAN (ID)"
    )
    vlan_vid = django_filters.NumberFilter(
        field_name="vlan__vid", label="VLAN number (1-4095)"
    )
    ...
    port_template = django_filters.NumberFilter(
        field_name="port_template__name", label="Port Template number (1-4095)"
    )
    ...
    status = django_filters.MultipleChoiceFilter(
        choices=PREFIX_STATUS_CHOICES, null_value=None
    )
    tag = TagFilter()
...
class IPAddressFilter(CustomFieldFilterSet, django_filters.FilterSet):
    ...
    def search(self, queryset, name, value):
        if not value.strip():
            return queryset
        qs_filter = Q(description__icontains=value) | Q(address__istartswith=value)
        return queryset.filter(qs_filter)
...
class VLANFilter(CustomFieldFilterSet, django_filters.FilterSet):
    ...
    group_id = django_filters.ModelMultipleChoiceFilter(
        queryset=VLANGroup.objects.all(), label="Group (ID)"
    )
    ...
    status = django_filters.MultipleChoiceFilter(
        choices=VLAN_STATUS_CHOICES, null_value=None
    )
    tag = TagFilter()
...
class ServiceFilter(django_filters.FilterSet):
    ...
    virtual_machine_id = django_filters.ModelMultipleChoiceFilter(
        queryset=VirtualMachine.objects.all(), label="Virtual machine (ID)"
    )
    virtual_machine = django_filters.ModelMultipleChoiceFilter(
