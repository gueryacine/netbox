...
class PrefixTable(BaseTable):
    ...
    port = tables.LinkColumn(
        "ipam:port", args=[Accessor("port.pk")], verbose_name="Port-templates"
    )
    ...
    class Meta(BaseTable.Meta):
        ...
        row_attrs = {"class": lambda record: "success" if not record.pk else ""}
...
class VLANTable(BaseTable):
    ...
    group = tables.LinkColumn(
        "ipam:vlangroup_vlans", args=[Accessor("group.pk")], verbose_name="Group"
    )
    tenant = tables.TemplateColumn(template_code=COL_TENANT)
    status = tables.TemplateColumn(STATUS_LABEL)
...
class VLANMemberTable(BaseTable):
    ...
    untagged = tables.TemplateColumn(
        template_code=VLAN_MEMBER_UNTAGGED, orderable=False
    )
    actions = tables.TemplateColumn(
...
class PortTemplateTable(BaseTable):
    ...
    site = tables.LinkColumn("dcim:site", args=[Accessor("site.slug")])
    group = tables.LinkColumn(
        "ipam:port_template_group_ports", args=[Accessor("group.pk")], verbose_name="Group"
    )
    tenant = tables.TemplateColumn(template_code=COL_TENANT)
    status = tables.TemplateColumn(STATUS_LABEL)
    ...
    class Meta(BaseTable.Meta):
        ...
        row_attrs = {
            "class": lambda record: "success" if not isinstance(record, PortTemplate) else ""
        }
...
class InterfacePortTemplateTable(BaseTable):
    ...
    vid = tables.LinkColumn("ipam:port", args=[Accessor("pk")], verbose_name="ID")
    tagged = BooleanColumn()
    site = tables.LinkColumn("dcim:site", args=[Accessor("site.slug")])
    group = tables.Column(accessor=Accessor("group.name"), verbose_name="Group")
    tenant = tables.TemplateColumn(template_code=COL_TENANT)
...
class ServiceTable(BaseTable):
    pk = ToggleColumn()
    name = tables.LinkColumn(viewname="ipam:service", args=[Accessor("pk")])
