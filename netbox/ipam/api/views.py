        ...
        if request.method == "POST":
            ...
            for i, requested_prefix in enumerate(requested_prefixes):
                ...
                try:
                    ...
                    return Response(
                        {
                            "detail": "Item {}: Invalid prefix length ({})".format(
                                i, prefix_length
                            )
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                ...
                if prefix.family == 4 and prefix_length > 32:
                    return Response(
                        {
                            "detail": "Item {}: Invalid prefix length ({}) for IPv4".format(
                                i, prefix_length
                            )
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    ...
                    return Response(
                        {
                            "detail": "Item {}: Invalid prefix length ({}) for IPv6".format(
                                i, prefix_length
                            )
                        },
                        status=status.HTTP_400_BAD_REQUEST,
            ...
            serializer = serializers.AvailablePrefixSerializer(
                ...
                many=True,
                context={"request": request, "vrf": prefix.vrf},
            )
...
class IPAddressViewSet(CustomFieldModelViewSet):
    queryset = IPAddress.objects.select_related(
        ...
        "tenant",
        "nat_inside",
        "interface__device__device_type",
        "interface__virtual_machine",
    ).prefetch_related("nat_outside", "tags")
