...
class AvailablePrefixSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        if self.context.get("vrf"):
            ...
            vrf = None
        return OrderedDict(
            [("family", instance.version), ("prefix", str(instance)), ("vrf", vrf)]
        )
...
class PortTemplatesGroupSerializer(ValidatedModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get("site", None):
            for field in ["name", "slug"]:
                validator = UniqueTogetherValidator(
                    queryset=PortTemplateGroup.objects.all(), fields=("site", field)
                )
                validator.set_context(self)
                validator(data)
...
class PortTemplatesSerializer(TaggitSerializer, CustomFieldModelSerializer):
    site = NestedSiteSerializer(required=False, allow_null=True)
    group = NestedPortTemplatesGroupSerializer(required=False, allow_null=True)
    tenant = NestedTenantSerializer(required=False, allow_null=True)
    ...
    def validate(self, data):
        ...
        if data.get("group", None):
            for field in ["name"]:
                validator = UniqueTogetherValidator(
                    queryset=PortTemplate.objects.all(), fields=("group", field)
                )
                validator.set_context(self)
                validator(data)
...
class IPAddressInterfaceSerializer(WritableNestedSerializer):
    ...
    class Meta:
        model = Interface
        fields = ["id", "url", "device", "virtual_machine", "name"]
    ...
    def get_url(self, obj):
        ...
        """
        url_name = (
            "dcim-api:interface-detail"
            if obj.device
            else "virtualization-api:interface-detail"
        )
