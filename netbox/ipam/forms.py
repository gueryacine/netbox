...
IP_FAMILY_CHOICES = [("", "All"), (4, "IPv4"), (6, "IPv6")]
...
class VRFForm(BootstrapMixin, TenancyForm, CustomFieldForm):
    ...
    class Meta:
        ...
        labels = {"rd": "RD"}
        help_texts = {"rd": "Route distinguisher in any format"}
...
class AggregateForm(BootstrapMixin, CustomFieldForm):
    ...
    class Meta:
        ...
        widgets = {"rir": APISelect(api_url="/api/ipam/rirs/")}
...
class AggregateFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    rir = FilterChoiceField(
        ...
        to_field_name="slug",
        label="RIR",
        widget=APISelectMultiple(api_url="/api/ipam/rirs/", value_field="slug"),
    )
...
class PrefixForm(BootstrapMixin, TenancyForm, CustomFieldForm):
    site = forms.ModelChoiceField(
        ...
        widget=APISelect(
            ...
            filter_for={"vlan_group": "site_id", "vlan": "site_id"},
            attrs={"nullable": "true"},
        ),
    )
    vlan_group = ChainedModelChoiceField(
        ...
        widget=APISelect(
            ...
            filter_for={"vlan": "group_id"},
            attrs={"nullable": "true"},
        ),
    )
    vlan = ChainedModelChoiceField(
        queryset=VLAN.objects.all(),
        chains=(("site", "site"), ("group", "vlan_group")),
        required=False,
        label="VLAN",
...
class PrefixCSVForm(forms.ModelForm):
    ...
    vlan_group = forms.CharField(
        help_text="Group name of assigned VLAN", required=False
    )
    vlan_vid = forms.IntegerField(
        help_text="Numeric ID of assigned VLAN", required=False
    )
    ...
    status = CSVChoiceField(
        choices=PREFIX_STATUS_CHOICES, help_text="Operational status"
    )
...
class PrefixBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    ...
    prefix_length = forms.IntegerField(min_value=1, max_value=127, required=False)
    tenant = forms.ModelChoiceField(
        queryset=Tenant.objects.all(),
        required=False,
        widget=APISelect(api_url="/api/tenancy/tenants/"),
...
class PrefixFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    vrf = FilterChoiceField(
        ...
        null_label="-- Global --",
        widget=APISelectMultiple(
            api_url="/api/ipam/vrfs/", value_field="rd", null_option=True
        ),
    )
    tenant = FilterChoiceField(
        queryset=Tenant.objects.all(),
    ...
    status = forms.MultipleChoiceField(
        choices=PREFIX_STATUS_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
...
class IPAddressForm(BootstrapMixin, TenancyForm, ReturnURLForm, CustomFieldForm):
    interface = forms.ModelChoiceField(queryset=Interface.objects.all(), required=False)
    nat_site = forms.ModelChoiceField(
        ...
        widget=APISelect(
            api_url="/api/dcim/sites/",
            filter_for={"nat_rack": "site_id", "nat_device": "site_id"},
        ),
    )
    ...
    nat_device = ChainedModelChoiceField(
        ...
        chains=(("site", "nat_site"), ("rack", "nat_rack")),
        required=False,
        label="Device",
        widget=APISelect(
    ...
    nat_inside = ChainedModelChoiceField(
        ...
        required=False,
        label="IP Address",
        widget=APISelect(api_url="/api/ipam/ip-addresses/", display_field="address"),
    )
    ...
    def clean(self):
        ...
        if self.cleaned_data.get("primary_for_parent") and not self.cleaned_data.get(
            "interface"
        ):
            self.add_error(
                "primary_for_parent",
...
class IPAddressCSVForm(forms.ModelForm):
    ...
    status = CSVChoiceField(
        choices=IPADDRESS_STATUS_CHOICES, help_text="Operational status"
    )
    role = CSVChoiceField(
        choices=IPADDRESS_ROLE_CHOICES, required=False, help_text="Functional role"
    )
    ...
    virtual_machine = forms.ModelChoiceField(
        ...
        error_messages={"invalid_choice": "Virtual machine not found."},
    )
    interface_name = forms.CharField(
        help_text="Name of assigned interface", required=False
    )
    ...
    def clean(self):
        ...
        if interface_name and device:
            try:
                ...
                raise forms.ValidationError(
                    "Invalid interface {} for device {}".format(interface_name, device)
                )
            ...
            raise forms.ValidationError(
                "Interface given ({}) but parent device/virtual machine not specified".format(
                    interface_name
                )
            )
        elif device:
    ...
    def save(self, *args, **kwargs):
        ...
        if self.cleaned_data["device"] and self.cleaned_data["interface_name"]:
            ...
            self.cleaned_data["virtual_machine"] and self.cleaned_data["interface_name"]
        ):
            self.instance.interface = Interface.objects.get(
...
class IPAddressBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    ...
    mask_length = forms.IntegerField(min_value=1, max_value=128, required=False)
    tenant = forms.ModelChoiceField(
        queryset=Tenant.objects.all(),
        required=False,
        widget=APISelect(api_url="/api/tenancy/tenants/"),
...
class IPAddressFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    status = forms.MultipleChoiceField(
        choices=IPADDRESS_STATUS_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
    role = forms.MultipleChoiceField(
        choices=IPADDRESS_ROLE_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
...
class VLANGroupCSVForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {"name": "Name of VLAN group"}
...
class VLANCSVForm(forms.ModelForm):
    ...
    group_name = forms.CharField(help_text="Name of VLAN group", required=False)
    tenant = forms.ModelChoiceField(
        queryset=Tenant.objects.all(),
        to_field_name="name",
        required=False,
    ...
    status = CSVChoiceField(choices=VLAN_STATUS_CHOICES, help_text="Operational status")
    role = forms.ModelChoiceField(
        queryset=Role.objects.all(),
        required=False,
        to_field_name="name",
    ...
    class Meta:
        ...
        help_texts = {"vid": "Numeric VLAN ID (1-4095)", "name": "VLAN name"}
...
class VLANFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    group_id = FilterChoiceField(
        ...
        label="VLAN group",
        null_label="-- None --",
        widget=APISelectMultiple(api_url="/api/ipam/vlan-groups/", null_option=True),
    )
    tenant = FilterChoiceField(
        queryset=Tenant.objects.all(),
        to_field_name="slug",
    ...
    status = forms.MultipleChoiceField(
        choices=VLAN_STATUS_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
    role = FilterChoiceField(
        queryset=Role.objects.all(),
        to_field_name="slug",
...
class PortTemplatesAssignVLANsForm(BootstrapMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Find all VLANs already assigned to the port for exclusion from the list
...
class PortTemplatesCSVForm(forms.ModelForm):
    ...
    status = CSVChoiceField(choices=PORT_STATUS_CHOICES, help_text="Operational status")
    types = CSVChoiceField(choices=PORT_TYPES_CHOICES, help_text="Operational status")
    role = forms.ModelChoiceField(
        queryset=Role.objects.all(),
        required=False,
        to_field_name="name",
    ...
    def clean(self):
        ...
        if group_name:
            try:
                self.instance.group = PortTemplateGroup.objects.get(site=site, name=group_name)
...
class PortTemplatesFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    group_id = FilterChoiceField(
        ...
        null_label="-- None --",
        widget=APISelectMultiple(api_url="/api/ipam/port-template-groups/", null_option=True),
    )
    tenant = FilterChoiceField(
        queryset=Tenant.objects.all(),
        to_field_name="slug",
    ...
    status = forms.MultipleChoiceField(
        choices=PORT_STATUS_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
    types = forms.MultipleChoiceField(
        choices=PORT_TYPES_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
    role = FilterChoiceField(
        queryset=Role.objects.all(),
        to_field_name="slug",
...
class ServiceForm(BootstrapMixin, CustomFieldForm):
    ...
    class Meta:
        ...
        widgets = {"protocol": StaticSelect2(), "ipaddresses": StaticSelect2Multiple()}
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.device:
            vc_interface_ids = [
                i["id"] for i in self.instance.device.vc_interfaces.values("id")
            ]
...
class ServiceBulkEditForm(BootstrapMixin, CustomFieldBulkEditForm):
    ...
    port = forms.IntegerField(
        validators=[MinValueValidator(1), MaxValueValidator(65535)], required=False
    )
    description = forms.CharField(max_length=100, required=False)

    class Meta:
        nullable_fields = ["site", "tenant", "role", "description"]
