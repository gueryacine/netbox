...
class PrefixListView(PermissionRequiredMixin, ObjectListView):
    ...
    queryset = Prefix.objects.prefetch_related('site', 'vrf__tenant', 'tenant','port_template', 'vlan', 'role')
...
class PrefixBulkEditView(PermissionRequiredMixin, BulkEditView):
    permission_required = 'ipam.change_prefix'
    queryset = Prefix.objects.prefetch_related('site', 'vrf__tenant', 'tenant', 'vlan','port_template', 'role')
    filterset = filters.PrefixFilterSet
    table = tables.PrefixTable
