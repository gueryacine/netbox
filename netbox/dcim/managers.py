...
SLOT_RE = (
    r"COALESCE(CAST(SUBSTRING({} FROM '^(?:[^0-9]+)?(\d{{1,9}})/') AS integer), NULL)"
)
...
CHANNEL_RE = (
    r"COALESCE(CAST(SUBSTRING({} FROM '^.*:(\d{{1,9}})(\.\d{{1,9}})?$') AS integer), 0)"
)
...
class DeviceComponentManager(Manager):
    def get_queryset(self):
        ...
        return queryset.extra(
            select={"name_padded": sql.format(table_name, table_name)}
...
class InterfaceManager(Manager):
    def get_queryset(self):
        ...
        return (
            InterfaceQuerySet(self.model, using=self._db)
            .annotate(**fields)
            .order_by(*ordering)
        )
