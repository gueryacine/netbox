...
from ipam.constants import PORT_TYPES_ETHERNET, PORT_TYPES_VIRTUAL, PORT_TYPES_LAG
...
class CableTermination(models.Model):
    ...
    def trace(self, position=1, follow_circuits=False, cable_history=None):
            ...
            # Map a rear port/position to its corresponding front port
                ...
                try:
                    peer_port = FrontPort.objects.get(
                        rear_port=termination, rear_port_position=position
                    )
                    return peer_port, 1
                except ObjectDoesNotExist:
                    return None, None
        ...
        far_end = (
            self.cable.termination_b
            if self.cable.termination_a == self
            else self.cable.termination_a
        )
...
class Region(MPTTModel, ChangeLoggedModel):
    ...
    def to_csv(self):
        return (self.name, self.slug, self.parent.name if self.parent else None)
...
class Site(ChangeLoggedModel, CustomFieldModel):
    ...
    status = models.PositiveSmallIntegerField(
        choices=SITE_STATUS_CHOICES, default=SITE_STATUS_ACTIVE
    )
...
class RackGroup(ChangeLoggedModel):
    ...
    def to_csv(self):
        return (self.site, self.name, self.slug)
...
class RackRole(ChangeLoggedModel):
    ...
    def to_csv(self):
        return (self.name, self.slug, self.color)
...
class Rack(ChangeLoggedModel, CustomFieldModel):
    ...
    status = models.PositiveSmallIntegerField(
        choices=RACK_STATUS_CHOICES, default=RACK_STATUS_ACTIVE
    )
    ...
    type = models.PositiveSmallIntegerField(
        choices=RACK_TYPE_CHOICES, blank=True, null=True, verbose_name="Type"
    )
    ...
    outer_unit = models.PositiveSmallIntegerField(
        choices=RACK_DIMENSION_UNIT_CHOICES, blank=True, null=True
    )
    comments = models.TextField(blank=True)
    custom_field_values = GenericRelation(
        to="extras.CustomFieldValue",
    ...
    def get_rack_units(
        self, face=RACK_FACE_FRONT, exclude=None, remove_redundant=False
    ):
        """
        ...
        if self.pk:
            for device in (
                Device.objects.select_related(
                    "device_type__manufacturer", "device_role"
                )
    ...
    def get_available_units(self, u_height=1, rack_face=None, exclude=list()):
        ...
        devices = (
            self.devices.select_related("device_type")
...
class DeviceType(ChangeLoggedModel, CustomFieldModel):
    ...
    def clean(self):
        ...
        if (
            self.subdevice_role != SUBDEVICE_ROLE_PARENT
...
class InterfaceTemplate(ComponentTemplateModel):
    ...
    form_factor = models.PositiveSmallIntegerField(
        choices=IFACE_FF_CHOICES, default=IFACE_FF_10GE_SFP_PLUS
    )
...
class FrontPortTemplate(ComponentTemplateModel):
    ...
    name = models.CharField(max_length=64)
    type = models.PositiveSmallIntegerField(choices=PORT_TYPE_CHOICES)
    ...
    rear_port_position = models.PositiveSmallIntegerField(
        default=1, validators=[MinValueValidator(1), MaxValueValidator(64)]
    )
...
class DeviceRole(ChangeLoggedModel):
    ...
    def to_csv(self):
        return (self.name, self.slug, self.color, self.vm_role)
...
class Device(ChangeLoggedModel, ConfigContextModel, CustomFieldModel):
    ...
    face = models.PositiveSmallIntegerField(
        blank=True, null=True, choices=RACK_FACE_CHOICES, verbose_name="Rack face"
    )
    status = models.PositiveSmallIntegerField(
    ...
    def clean(self):
        ...
        if self.primary_ip4:
            if self.primary_ip4.interface in vc_interfaces:
                ...
                and self.primary_ip4.nat_inside.interface in vc_interfaces
            ):
                pass
            else:
                raise ValidationError(
        ...
        if self.primary_ip6:
            if self.primary_ip6.interface in vc_interfaces:
                ...
                and self.primary_ip6.nat_inside.interface in vc_interfaces
            ):
                pass
            else:
                raise ValidationError(
        ...
        # # Validate PORT group
        # if self.port_groups and self.port_groups.site != self.site:
        #     raise ValidationError({
        #         'group': "PORT group must belong to the assigned site ({}).".format(self.site)
        #     })
        ...
        if is_new:
            ConsolePort.objects.bulk_create(
                [
                    ...
                    for template in self.device_type.consoleport_templates.all()
                ]
            )
            ConsoleServerPort.objects.bulk_create(
                [
                    ConsoleServerPort(device=self, name=template.name)
                    for template in self.device_type.consoleserverport_templates.all()
                ]
            )
            PowerPort.objects.bulk_create(
                [
                    PowerPort(device=self, name=template.name)
                    for template in self.device_type.powerport_templates.all()
                ]
            )
            PowerOutlet.objects.bulk_create(
                [
                    PowerOutlet(device=self, name=template.name)
                    for template in self.device_type.poweroutlet_templates.all()
                ]
            )
                ...
                [
                    FrontPort(
                        ...
                        rear_port=RearPort.objects.get(
                            device=self, name=template.rear_port.name
                        ),
...
class ConsolePort(CableTermination, ComponentModel):
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, related_name="consoleports"
    )
    name = models.CharField(max_length=50)
    ...
    def to_csv(self):
        return (self.device.identifier, self.name)
...
class ConsoleServerPort(CableTermination, ComponentModel):
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, related_name="consoleserverports"
    )
    name = models.CharField(max_length=50)
    connection_status = models.NullBooleanField(
        choices=CONNECTION_STATUS_CHOICES, blank=True
...
class PowerPort(CableTermination, ComponentModel):
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, related_name="powerports"
    )
    name = models.CharField(max_length=50)
    connected_endpoint = models.OneToOneField(
        to="dcim.PowerOutlet",
...
class PowerOutlet(CableTermination, ComponentModel):
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, related_name="poweroutlets"
    )
    name = models.CharField(max_length=50)
    connection_status = models.NullBooleanField(
        choices=CONNECTION_STATUS_CHOICES, blank=True
...
class Interface(CableTermination, ComponentModel):
    ...
    def clean(self):
        ...
        if self.port_template:
            if (
                (self.port_template.types != PORT_TYPES_ETHERNET and self.is_ethernet)
                or (self.port_template.types != PORT_TYPES_LAG and self.is_lag)
                or (self.port_template.types != PORT_TYPES_VIRTUAL and self.is_virtual)
            ):
                raise ValidationError(
                    {
...
class FrontPort(CableTermination, ComponentModel):
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, related_name="frontports"
    )
    name = models.CharField(max_length=64)
    type = models.PositiveSmallIntegerField(choices=PORT_TYPE_CHOICES)
    rear_port = models.ForeignKey(
...
class RearPort(CableTermination, ComponentModel):
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, related_name="rearports"
    )
    name = models.CharField(max_length=64)
    type = models.PositiveSmallIntegerField(choices=PORT_TYPE_CHOICES)
    positions = models.PositiveSmallIntegerField(
...
class DeviceBay(ComponentModel):
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, related_name="device_bays"
    )
...
class InventoryItem(ComponentModel):
    ...
    device = models.ForeignKey(
        to="dcim.Device", on_delete=models.CASCADE, related_name="inventory_items"
    )
...
class VirtualChassis(ChangeLoggedModel):
    ...
    master = models.OneToOneField(
        to="Device", on_delete=models.PROTECT, related_name="vc_master_for"
    )
    ...
    def to_csv(self):
        return (self.master, self.domain)
...
class Cable(ChangeLoggedModel):
    ...
    type = models.PositiveSmallIntegerField(
        choices=CABLE_TYPE_CHOICES, blank=True, null=True
    )
    status = models.BooleanField(
        choices=CONNECTION_STATUS_CHOICES, default=CONNECTION_STATUS_CONNECTED
    )
    label = models.CharField(max_length=100, blank=True)
    color = ColorField(blank=True)
    length = models.PositiveSmallIntegerField(blank=True, null=True)
    length_unit = models.PositiveSmallIntegerField(
        choices=CABLE_LENGTH_UNIT_CHOICES, blank=True, null=True
    )
        ...
        if self.termination_a and self.termination_b:
            ...
            if (
                ...
                and (
                    ...
                    or getattr(self.termination_b, "rear_port", None)
                    == self.termination_a
                )
            ):
                raise ValidationError(
            ...
            if (
                isinstance(endpoint_a, Interface)
                and endpoint_a.form_factor == IFACE_FF_VIRTUAL
            ) or (
                isinstance(endpoint_b, Interface)
                and endpoint_b.form_factor == IFACE_FF_VIRTUAL
            ):
