...
class RegionFilter(NameSlugSearchFilterSet):
    parent_id = django_filters.ModelMultipleChoiceFilter(
        queryset=Region.objects.all(), label="Parent region (ID)"
    )
...
class SiteFilter(CustomFieldFilterSet, django_filters.FilterSet):
    ...
    status = django_filters.MultipleChoiceFilter(
        choices=SITE_STATUS_CHOICES, null_value=None
    )
    region_id = django_filters.NumberFilter(
        method="filter_region", field_name="pk", label="Region (ID)"
    )
    region = django_filters.CharFilter(
        method="filter_region", field_name="slug", label="Region (slug)"
    )
...
class RackFilter(CustomFieldFilterSet, django_filters.FilterSet):
    ...
    group_id = django_filters.ModelMultipleChoiceFilter(
        queryset=RackGroup.objects.all(), label="Group (ID)"
    )
    ...
    status = django_filters.MultipleChoiceFilter(
        choices=RACK_STATUS_CHOICES, null_value=None
    )
    role_id = django_filters.ModelMultipleChoiceFilter(
        queryset=RackRole.objects.all(), label="Role (ID)"
    )
...
class RackReservationFilter(django_filters.FilterSet):
    ...
    site_id = django_filters.ModelMultipleChoiceFilter(
        field_name="rack__site", queryset=Site.objects.all(), label="Site (ID)"
    )
    site = django_filters.ModelMultipleChoiceFilter(
    ...
    group_id = django_filters.ModelMultipleChoiceFilter(
        field_name="rack__group", queryset=RackGroup.objects.all(), label="Group (ID)"
    )
    group = django_filters.ModelMultipleChoiceFilter(
...
class DeviceTypeFilter(CustomFieldFilterSet):
    ...
    manufacturer_id = django_filters.ModelMultipleChoiceFilter(
        queryset=Manufacturer.objects.all(), label="Manufacturer (ID)"
    )
    ...
    console_ports = django_filters.BooleanFilter(
        method="_console_ports", label="Has console ports"
    )
    console_server_ports = django_filters.BooleanFilter(
        method="_console_server_ports", label="Has console server ports"
    )
    power_ports = django_filters.BooleanFilter(
        method="_power_ports", label="Has power ports"
    )
    power_outlets = django_filters.BooleanFilter(
        method="_power_outlets", label="Has power outlets"
    )
    interfaces = django_filters.BooleanFilter(
        method="_interfaces", label="Has interfaces"
    )
    pass_through_ports = django_filters.BooleanFilter(
        method="_pass_through_ports", label="Has pass-through ports"
    )
    tag = TagFilter()
    ...
    def _pass_through_ports(self, queryset, name, value):
        return queryset.exclude(
            frontport_templates__isnull=value, rearport_templates__isnull=value
        )
...
class DeviceFilter(CustomFieldFilterSet):
    ...
    rack_id = django_filters.ModelMultipleChoiceFilter(
        field_name="rack", queryset=Rack.objects.all(), label="Rack (ID)"
    )
    cluster_id = django_filters.ModelMultipleChoiceFilter(
        queryset=Cluster.objects.all(), label="VM cluster (ID)"
    )
    ...
    status = django_filters.MultipleChoiceFilter(
        choices=DEVICE_STATUS_CHOICES, null_value=None
    )
    is_full_depth = django_filters.BooleanFilter(
        field_name="device_type__is_full_depth", label="Is full depth"
    )
    mac_address = django_filters.CharFilter(method="_mac_address", label="MAC address")
    has_primary_ip = django_filters.BooleanFilter(
        method="_has_primary_ip", label="Has a primary IP"
    )
    ...
    def filter_region(self, queryset, name, value):
        try:
            ...
            return queryset.none()
        return queryset.filter(
            Q(site__region=region) | Q(site__region__in=region.get_descendants())
        )
...
class DeviceComponentFilterSet(django_filters.FilterSet):
    ...
    device = django_filters.ModelChoiceFilter(
        queryset=Device.objects.all(), to_field_name="name", label="Device (name)"
    )
    tag = TagFilter()
...
class InterfaceFilter(django_filters.FilterSet):
    ...
    device = django_filters.CharFilter(
        method="filter_device", field_name="name", label="Device"
    )
    device_id = django_filters.NumberFilter(
        method="filter_device", field_name="pk", label="Device (ID)"
    )
    cabled = django_filters.BooleanFilter(
        field_name="cable", lookup_expr="isnull", exclude=True
    )
    type = django_filters.CharFilter(method="filter_type", label="Interface type")
    lag_id = django_filters.ModelMultipleChoiceFilter(
        field_name="lag", queryset=Interface.objects.all(), label="LAG interface (ID)"
    )
    mac_address = django_filters.CharFilter(method="_mac_address", label="MAC address")
    tag = TagFilter()
    ...
    form_factor = django_filters.MultipleChoiceFilter(
        choices=IFACE_FF_CHOICES, null_value=None
    )
...
class VirtualChassisFilter(django_filters.FilterSet):
    q = django_filters.CharFilter(method="search", label="Search")
    site_id = django_filters.ModelMultipleChoiceFilter(
        field_name="master__site", queryset=Site.objects.all(), label="Site (ID)"
    )
    site = django_filters.ModelMultipleChoiceFilter(
    ...
    tenant_id = django_filters.ModelMultipleChoiceFilter(
        field_name="master__tenant", queryset=Tenant.objects.all(), label="Tenant (ID)"
    )
    tenant = django_filters.ModelMultipleChoiceFilter(
    ...
    def search(self, queryset, name, value):
        if not value.strip():
            return queryset
        qs_filter = Q(master__name__icontains=value) | Q(domain__icontains=value)
        return queryset.filter(qs_filter)


class CableFilter(django_filters.FilterSet):
    q = django_filters.CharFilter(method="search", label="Search")
    type = django_filters.MultipleChoiceFilter(choices=CABLE_TYPE_CHOICES)
    color = django_filters.MultipleChoiceFilter(choices=COLOR_CHOICES)
...
class ConsoleConnectionFilter(django_filters.FilterSet):
    site = django_filters.CharFilter(method="filter_site", label="Site (slug)")
    device = django_filters.CharFilter(method="filter_device", label="Device")
