...
from ipam.tables import (
    ...
    InterfaceVLANTable,
    PortTemplateVLANTable,
    InterfacePortTemplateTable,
)
            ...
            if form.is_valid():
                ...
                if "_apply" in request.POST:
                    ...
                    messages.success(
                        request,
                        "Renamed {} {}".format(
                            len(selected_objects), model._meta.verbose_name_plural
                        ),
                    )
...
class SiteView(View):
    def get(self, request, slug):
        ...
        site = get_object_or_404(
            Site.objects.select_related("region", "tenant__group"), slug=slug
        )
        stats = {
        ...
        rack_groups = RackGroup.objects.filter(site=site).annotate(
...
class RackView(View):
    def get(self, request, pk):
        ...
        next_rack = (
            Rack.objects.filter(site=rack.site, name__gt=rack.name)
            .order_by("name")
            .first()
        )
        prev_rack = (
            Rack.objects.filter(site=rack.site, name__lt=rack.name)
            .order_by("-name")
            .first()
        )
...
class DeviceTypeView(View):
    def get(self, request, pk):
        ...
        consoleport_table = tables.ConsolePortTemplateTable(
            ConsolePortTemplate.objects.filter(device_type=devicetype), orderable=False
        )
        ...
        powerport_table = tables.PowerPortTemplateTable(
            PowerPortTemplate.objects.filter(device_type=devicetype), orderable=False
        )
        poweroutlet_table = tables.PowerOutletTemplateTable(
            PowerOutletTemplate.objects.filter(device_type=devicetype), orderable=False
        )
        ...
        front_port_table = tables.FrontPortTemplateTable(
            FrontPortTemplate.objects.filter(device_type=devicetype), orderable=False
        )
        rear_port_table = tables.RearPortTemplateTable(
            RearPortTemplate.objects.filter(device_type=devicetype), orderable=False
        )
        devicebay_table = tables.DeviceBayTemplateTable(
            DeviceBayTemplate.objects.filter(device_type=devicetype), orderable=False
        )
...
class DeviceView(View):
    def get(self, request, pk):
        ...
        # Console ports
        console_ports = device.consoleports.select_related(
            "connected_endpoint__device", "cable"
        )

        # Console server ports
        consoleserverports = device.consoleserverports.select_related(
            "connected_endpoint__device", "cable"
        )

        # Power ports
        power_ports = device.powerports.select_related(
            "connected_endpoint__device", "cable"
        )

        # Power outlets
        poweroutlets = device.poweroutlets.select_related(
            "connected_endpoint__device", "cable"
        )
        ...
        # Device bays
        device_bays = device.device_bays.select_related(
        ...
        related_devices = (
            Device.objects.filter(site=device.site, device_role=device.device_role)
...
class DeviceInventoryView(View):
    def get(self, request, pk):
        ...
        inventory_items = (
            InventoryItem.objects.filter(device=device, parent=None)
...
class DeviceStatusView(PermissionRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(
            ...
            {"device": device, "active_tab": "status"},
        )
...
class DeviceConfigView(PermissionRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(
            ...
            {"device": device, "active_tab": "config"},
        )
...
class InterfaceView(View):
    def get(self, request, pk):
        ...
        ipaddress_table = InterfaceIPAddressTable(
            data=interface.ip_addresses.select_related("vrf", "tenant"), orderable=False
        )
        ...
        if interface.port_template is not None:
            ...
            for vlan in interface.tagged_vlans.select_related(
                "site", "group", "tenant", "role"
            ):
                vlan.tagged = True
                vlans.append(vlan)
...
class DeviceBulkAddConsoleServerPortView(
...
class CableView(View):
    def get(self, request, pk):
        ...
        return render(request, "dcim/cable.html", {"cable": cable})
...
class CableTraceView(View):
    ...
    def get(self, request, model, pk):
        ...
        return render(
            ...
            {"obj": obj, "trace": obj.trace(follow_circuits=True)},
        )
...
class ConsoleConnectionsListView(ObjectListView):
    queryset = (
        ConsolePort.objects.select_related("device", "connected_endpoint__device")
...
class PowerConnectionsListView(ObjectListView):
    queryset = (
        PowerPort.objects.select_related("device", "connected_endpoint__device")
        .filter(connected_endpoint__isnull=False)
        .order_by(
            "cable", "connected_endpoint__device__name", "connected_endpoint__name"
        )
        ...
        csv_data = [
            ...
            ",".join(
                [
                    "device_a",
                    "interface_a",
                    "interface_a_description",
                    "device_b",
                    "interface_b",
                    "interface_b_description",
                    "connection_status",
                ]
            )
        ]
        for obj in self.queryset:
            csv = csv_format(
                [
                    ...
                    obj.connected_endpoint.name if obj.connected_endpoint else None,
                    obj.connected_endpoint.description
                    if obj.connected_endpoint
                    else None,
                    obj.device.identifier,
                    obj.name,
...
class VirtualChassisCreateView(PermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        device_queryset = (
            Device.objects.filter(pk__in=pk_form.cleaned_data.get("pk"))
            .select_related("rack")
            .order_by("vc_position")
        )
...
class VirtualChassisEditView(PermissionRequiredMixin, GetReturnURLMixin, View):
    ...
    def get(self, request, pk):
        ...
        members_queryset = virtual_chassis.members.select_related("rack").order_by(
            "vc_position"
        )
    ...
    def post(self, request, pk):
        ...
        if vc_form.is_valid() and formset.is_valid():
            ...
            with transaction.atomic():
                ...
                Device.objects.filter(pk__in=[m.pk for m in members]).update(
                    vc_position=None
                )
...
# class DeviceAssignPORTsGroupsView(ObjectEditView):
#     permission_required = 'ipam.add_service'

#  class ImageAttachmentEditView(PermissionRequiredMixin, ObjectEditView):
#     permission_required = 'extras.change_imageattachment'
#     model = PORTGroup
#     model_form = forms.DeviceAssignPORTsGroupsForm
