...
class RegionForm(BootstrapMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {"parent": APISelect(api_url="/api/dcim/regions/")}
...
class RegionCSVForm(forms.ModelForm):
    parent = forms.ModelChoiceField(
        ...
        error_messages={"invalid_choice": "Region not found."},
    )

    class Meta:
        model = Region
        fields = Region.csv_headers
        help_texts = {"name": "Region name", "slug": "URL-friendly slug"}
...
class SiteForm(BootstrapMixin, TenancyForm, CustomFieldForm):
    region = TreeNodeChoiceField(
        queryset=Region.objects.all(),
        required=False,
        widget=APISelect(api_url="/api/dcim/regions/"),
    )
    slug = SlugField()
...
class SiteFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    status = forms.MultipleChoiceField(
        choices=SITE_STATUS_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
    region = forms.ModelMultipleChoiceField(
        queryset=Region.objects.all(),
        to_field_name="slug",
        required=False,
        widget=APISelectMultiple(api_url="/api/dcim/regions/", value_field="slug"),
    )
    tenant = FilterChoiceField(
        ...
        widget=APISelectMultiple(
            api_url="/api/tenancy/tenants/", value_field="slug", null_option=True
        ),
    )
...
class RackGroupCSVForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {"name": "Name of rack group", "slug": "URL-friendly slug"}
...
class RackGroupFilterForm(BootstrapMixin, forms.Form):
    site = FilterChoiceField(
        queryset=Site.objects.all(),
        to_field_name="slug",
        widget=APISelectMultiple(api_url="/api/dcim/sites/", value_field="slug"),
    )
...
class RackForm(BootstrapMixin, TenancyForm, CustomFieldForm):
    group = ChainedModelChoiceField(
        ...
        chains=(("site", "site"),),
        required=False,
        widget=APISelect(api_url="/api/dcim/rack-groups/"),
    )
    comments = CommentField()
    tags = TagField(required=False)
    ...
    class Meta:
        ...
        widgets = {
            "site": APISelect(
                api_url="/api/dcim/sites/", filter_for={"group": "site_id"}
            ),
            "status": StaticSelect2(),
            "role": APISelect(api_url="/api/dcim/rack-roles/"),
            "type": StaticSelect2(),
            "width": StaticSelect2(),
...
class RackCSVForm(forms.ModelForm):
    ...
    group_name = forms.CharField(help_text="Name of rack group", required=False)
    tenant = forms.ModelChoiceField(
        queryset=Tenant.objects.all(),
        required=False,
        to_field_name="name",
    ...
    status = CSVChoiceField(
        choices=RACK_STATUS_CHOICES, required=False, help_text="Operational status"
    )
    ...
    type = CSVChoiceField(
        choices=RACK_TYPE_CHOICES, required=False, help_text="Rack type"
    )
    width = forms.ChoiceField(
        choices=((RACK_WIDTH_19IN, "19"), (RACK_WIDTH_23IN, "23")),
    ...
    class Meta:
        ...
        help_texts = {"name": "Rack name", "u_height": "Height in rack units"}
    ...
    def clean(self):
        ...
        if group_name:
            ...
            if Rack.objects.filter(group=self.instance.group, name=name).exists():
                raise forms.ValidationError(
                    "A rack named {} already exists within group {}".format(
                        name, group_name
                    )
                )
            ...
            if (
                facility_id
                and Rack.objects.filter(
                    group=self.instance.group, facility_id=facility_id
                ).exists()
            ):
                raise forms.ValidationError(
                    "A rack with the facility ID {} already exists within group {}".format(
                        facility_id, group_name
                    )
                )
...
class RackBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    ...
    site = forms.ModelChoiceField(
        queryset=Site.objects.all(),
        required=False,
        widget=APISelect(api_url="/api/dcim/sites", filter_for={"group": "site_id"}),
    )
    ...
    role = forms.ModelChoiceField(
        queryset=RackRole.objects.all(),
        required=False,
        widget=APISelect(api_url="/api/dcim/rack-roles"),
    )
...
class RackFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    group_id = FilterChoiceField(
        ...
        label="Rack group",
        null_label="-- None --",
        widget=APISelectMultiple(api_url="/api/dcim/rack-groups/", null_option=True),
    )
    tenant = FilterChoiceField(
        queryset=Tenant.objects.all(),
        to_field_name="slug",
    ...
    status = forms.MultipleChoiceField(
        choices=RACK_STATUS_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
    role = FilterChoiceField(
        ...
        null_label="-- None --",
        widget=APISelectMultiple(
            api_url="/api/dcim/rack-roles/", value_field="slug", null_option=True
        ),
    )
...
class RackReservationBulkEditForm(BootstrapMixin, BulkEditForm):
    ...
    tenant = forms.ModelChoiceField(
        queryset=Tenant.objects.all(),
        required=False,
        widget=APISelect(api_url="/api/tenancy/tenant"),
    )
    description = forms.CharField(max_length=100, required=False)
...
class ManufacturerCSVForm(forms.ModelForm):
    class Meta:
        ...
        help_texts = {"name": "Manufacturer name", "slug": "URL-friendly slug"}
...
class DeviceTypeForm(BootstrapMixin, CustomFieldForm):
    ...
    class Meta:
        ...
        widgets = {
            "manufacturer": APISelect(api_url="/api/dcim/manufacturers/"),
...
class DeviceTypeCSVForm(forms.ModelForm):
    manufacturer = forms.ModelChoiceField(
        ...
        error_messages={"invalid_choice": "Manufacturer not found."},
    )
    subdevice_role = CSVChoiceField(
        choices=SUBDEVICE_ROLE_CHOICES, required=False, help_text="Parent/child status"
    )

    class Meta:
        model = DeviceType
        fields = DeviceType.csv_headers
        help_texts = {"model": "Model name", "slug": "URL-friendly slug"}
...
class DeviceTypeBulkEditForm(
    ...
    manufacturer = forms.ModelChoiceField(
        queryset=Manufacturer.objects.all(),
        required=False,
        widget=APISelect(api_url="/api/dcim/manufactureres"),
    )
...
class DeviceTypeFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    manufacturer = FilterChoiceField(
        ...
        to_field_name="slug",
        widget=APISelectMultiple(
            api_url="/api/dcim/manufacturers/", value_field="slug"
        ),
    )
    subdevice_role = forms.NullBooleanField(
        required=False,
        label="Subdevice role",
        widget=StaticSelect2(choices=add_blank_choice(SUBDEVICE_ROLE_CHOICES)),
    )
...
class InterfaceTemplateForm(BootstrapMixin, forms.ModelForm):
    class Meta:
        ...
        widgets = {"device_type": forms.HiddenInput(), "form_factor": StaticSelect2()}


class InterfaceTemplateCreateForm(ComponentForm):
    name_pattern = ExpandableNameField(label="Name")
    form_factor = forms.ChoiceField(choices=IFACE_FF_CHOICES, widget=StaticSelect2())
...
class FrontPortTemplateForm(BootstrapMixin, forms.ModelForm):
    class Meta:
        ...
        widgets = {"device_type": forms.HiddenInput(), "rear_port": StaticSelect2()}


class FrontPortTemplateCreateForm(ComponentForm):
    name_pattern = ExpandableNameField(label="Name")
    type = forms.ChoiceField(choices=PORT_TYPE_CHOICES, widget=StaticSelect2())
...
class RearPortTemplateForm(BootstrapMixin, forms.ModelForm):
    class Meta:
        ...
        widgets = {"device_type": forms.HiddenInput(), "type": StaticSelect2()}
...
class DeviceForm(BootstrapMixin, TenancyForm, CustomFieldForm):
    site = forms.ModelChoiceField(
        queryset=Site.objects.all(),
        widget=APISelect(api_url="/api/dcim/sites/", filter_for={"rack": "site_id"}),
    )
    rack = ChainedModelChoiceField(
        queryset=Rack.objects.all(),
        chains=(("site", "site"),),
        required=False,
        widget=APISelect(api_url="/api/dcim/racks/", display_field="display_name"),
    )
    position = forms.TypedChoiceField(
        ...
        widget=APISelect(
            api_url="/api/dcim/racks/{{rack}}/units/", disabled_indicator="device"
        ),
    )
    manufacturer = forms.ModelChoiceField(
        queryset=Manufacturer.objects.all(),
        widget=APISelect(
            ...
            filter_for={"device_type": "manufacturer_id"},
        ),
    )
    device_type = ChainedModelChoiceField(
        ...
        label="Device type",
        widget=APISelect(api_url="/api/dcim/device-types/", display_field="model"),
    )
    ...
    class Meta:
        ...
        widgets = {
            "face": StaticSelect2(filter_for={"position": "face"}),
            "device_role": APISelect(api_url="/api/dcim/device-roles/"),
            "status": StaticSelect2(),
            "platform": APISelect(api_url="/api/dcim/platforms/"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            for family in [4, 6]:
                ...
                if nat_ips:
                    ip_list = [
                        (ip.id, "{} ({})".format(ip.address, ip.nat_inside.address))
                        for ip in nat_ips
                    ]
            ...
            self.fields["position"].widget.add_additional_query_param(
...
class BaseDeviceCSVForm(forms.ModelForm):
    ...
    model_name = forms.CharField(help_text="Device type model name")
    ...
    status = CSVChoiceField(
        choices=DEVICE_STATUS_CHOICES, help_text="Operational status"
    )

    class Meta:
        fields = []
        model = Device
        help_texts = {"name": "Device name"}
...
class DeviceCSVForm(BaseDeviceCSVForm):
    site = forms.ModelChoiceField(
        ...
        to_field_name="name",
        help_text="Name of parent site",
        error_messages={"invalid_choice": "Invalid site name."},
    )
    rack_group = forms.CharField(
        required=False, help_text="Parent rack's group (if any)"
    )
    rack_name = forms.CharField(required=False, help_text="Name of parent rack")
    face = CSVChoiceField(
        choices=RACK_FACE_CHOICES, required=False, help_text="Mounted rack face"
    )
    cluster = forms.ModelChoiceField(
        ...
        error_messages={"invalid_choice": "Invalid cluster name."},
    )
    ...
    def clean(self):
        ...
        if site and rack_group and rack_name:
            try:
                ...
                raise forms.ValidationError(
                    "Rack {} not found in site {} group {}".format(
                        rack_name, site, rack_group
                    )
                )
...
class ChildDeviceCSVForm(BaseDeviceCSVForm):
    ...
    device_bay_name = forms.CharField(help_text="Name of device bay")
    cluster = forms.ModelChoiceField(
        queryset=Cluster.objects.all(),
        to_field_name="name",
        required=False,
    ...
    def clean(self):
        ...
        if parent and device_bay_name:
            try:
                ...
                raise forms.ValidationError(
                    "Parent device/bay ({} {}) not found".format(
...
class DeviceBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    ...
    device_type = forms.ModelChoiceField(
        ...
        label="Type",
        widget=APISelect(
            api_url="/api/dcim/device-types/", display_field="display_name"
        ),
    )
    device_role = forms.ModelChoiceField(
        queryset=DeviceRole.objects.all(),
...
class DeviceFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    site = FilterChoiceField(
        ...
        widget=APISelectMultiple(
            ...
            value_field="slug",
            filter_for={"rack_group_id": "site", "rack_id": "site"},
        ),
    )
    rack_group_id = FilterChoiceField(
        ...
        label="Rack group",
        widget=APISelectMultiple(
            api_url="/api/dcim/rack-groups/", filter_for={"rack_id": "rack_group_id"}
        ),
    )
    rack_id = FilterChoiceField(
        ...
        label="Rack",
        null_label="-- None --",
        widget=APISelectMultiple(api_url="/api/dcim/racks/", null_option=True),
    )
    role = FilterChoiceField(
        queryset=DeviceRole.objects.all(),
        to_field_name="slug",
        widget=APISelectMultiple(
            api_url="/api/dcim/device-roles/", value_field="slug", null_option=True
        ),
    )
    tenant = FilterChoiceField(
        queryset=Tenant.objects.all(),
    ...
    manufacturer_id = FilterChoiceField(
        ...
        widget=APISelectMultiple(
            api_url="/api/dcim/manufacturers/",
            filter_for={"device_type_id": "manufacturer_id"},
        ),
    )
    device_type_id = FilterChoiceField(
        ...
        label="Model",
        widget=APISelectMultiple(
            api_url="/api/dcim/device-types/", display_field="model"
        ),
    )
    platform = FilterChoiceField(
        ...
        null_label="-- None --",
        widget=APISelectMultiple(
            api_url="/api/dcim/platforms/", value_field="slug", null_option=True
        ),
    )
    status = forms.MultipleChoiceField(
        choices=DEVICE_STATUS_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
...
class InterfaceForm(BootstrapMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {"mode": "802.1Q Mode", "port_template": "PortTemplate"}
        help_texts = {"mode": INTERFACE_MODE_HELP_TEXT}
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.is_bound:
            ...
            self.fields["port_template"].queryset = PortTemplate.objects.filter(
                group__in=[port_template_group for port_template_group in device.port_template_groups.all()]
            )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.instance.port_template is not None:
            ...
            vlans_tagged =  VLAN.objects.filter(
                id__in=[untagged_vlan.id for untagged_vlan in self.instance.port_template.tagged_vlans.all()]
            )
...
class InterfaceAssignVLANsForm(BootstrapMixin, forms.ModelForm):
    vlans = forms.MultipleChoiceField(
        choices=[], label="VLANs", widget=StaticSelect2Multiple(attrs={"size": 20})
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        vlan_choices.append(("Global", [(vlan.pk, vlan) for vlan in global_vlans]))
...
class InterfaceCreateForm(ComponentForm, forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.parent is not None:
            ...
            self.fields["port_template"].queryset = PortTemplate.objects.filter(
                group__in=[port_template_group for port_template_group in self.parent.port_template_groups.all()]
            )
...
class InterfaceBulkEditForm(BootstrapMixin, AddRemoveTagsForm, BulkEditForm):
    ...
    class Meta:
        nullable_fields = ["lag", "mac_address", "mtu", "description", "mode"]
...
class RearPortForm(BootstrapMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {"device": forms.HiddenInput(), "type": StaticSelect2()}
...
class CableCreateForm(BootstrapMixin, ChainedFieldsMixin, forms.ModelForm):
    ...
    termination_b_rack = ChainedModelChoiceField(
        queryset=Rack.objects.all(),
        chains=(("site", "termination_b_site"),),
        label="Rack",
        required=False,
        widget=APISelect(
            api_url="/api/dcim/racks/",
            filter_for={"termination_b_device": "rack_id"},
            attrs={"nullable": "true"},
        ),
    )
    termination_b_device = ChainedModelChoiceField(
        queryset=Device.objects.all(),
        chains=(("site", "termination_b_site"), ("rack", "termination_b_rack")),
        label="Device",
        required=False,
        widget=APISelect(
            api_url="/api/dcim/devices/",
            display_field="display_name",
            filter_for={"termination_b_id": "device_id"},
        ),
    )
    termination_b_type = forms.ModelChoiceField(
        queryset=ContentType.objects.all(), label="Type", widget=ContentTypeSelect()
    )
    termination_b_id = forms.IntegerField(
        ...
        widget=APISelect(
            ...
            conditional_query_params={"termination_b_type__interface": "type=physical"},
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        ).exclude(model="circuittermination")
...
class CableCSVForm(forms.ModelForm):
    ...
    side_a_device = FlexibleModelChoiceField(
        ...
        error_messages={"invalid_choice": "Side A device not found"},
    )
    ...
    side_a_name = forms.CharField(help_text="Side A component")
    ...
    side_b_device = FlexibleModelChoiceField(
        ...
        error_messages={"invalid_choice": "Side B device not found"},
    )
    ...
    side_b_name = forms.CharField(help_text="Side B component")

    # Cable attributes
    status = CSVChoiceField(
        choices=CONNECTION_STATUS_CHOICES, required=False, help_text="Connection status"
    )
    type = CSVChoiceField(
        choices=CABLE_TYPE_CHOICES, required=False, help_text="Cable type"
    )
    length_unit = CSVChoiceField(
        choices=CABLE_LENGTH_UNIT_CHOICES, required=False, help_text="Length unit"
    )
    ...
    class Meta:
        ...
        help_texts = {"color": "RGB color in hexadecimal (e.g. 00ff00)"}
...
class CableBulkEditForm(BootstrapMixin, BulkEditForm):
    ...
    status = forms.ChoiceField(
        choices=add_blank_choice(CONNECTION_STATUS_CHOICES), required=False, initial=""
    )
    label = forms.CharField(max_length=100, required=False, widget=StaticSelect2())
...
class CableFilterForm(BootstrapMixin, forms.Form):
    ...
    type = forms.MultipleChoiceField(
        choices=CABLE_TYPE_CHOICES, required=False, widget=StaticSelect2()
    )
    color = forms.CharField(max_length=6, required=False, widget=ColorSelect())
...
class InventoryItemForm(BootstrapMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {"manufacturer": APISelect(api_url="/api/dcim/manufacturers/")}
...
class DeviceVCMembershipForm(forms.ModelForm):
    class Meta:
        ...
        labels = {"vc_position": "Position", "vc_priority": "Priority"}
...
class VCMemberSelectForm(BootstrapMixin, ChainedFieldsMixin, forms.Form):
    site = forms.ModelChoiceField(
        ...
        widget=APISelect(
            api_url="/api/dcim/sites/",
            filter_for={"rack": "site_id", "device": "site_id"},
        ),
    )
    rack = ChainedModelChoiceField(
        queryset=Rack.objects.all(),
...
class VirtualChassisFilterForm(BootstrapMixin):
    model = VirtualChassis
    q = forms.CharField(required=False, label="Search")
    site = FilterChoiceField(queryset=Site.objects.all(), to_field_name="slug")
    tenant = FilterChoiceField(
        queryset=Tenant.objects.all(), to_field_name="slug", null_label="-- None --"
    )
...
class DeviceAssignPortTemplatesGroupsForm(forms.Form, CustomFieldForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.initial["port_template_groups"] = [v.pk for v in self.instance.port_template_groups.all()]

        # Compile VLAN choices
        port_template_groups_choices = []

        global_port_template_groups = PortTemplateGroup.objects.filter(site=None)
        port_template_groups_choices.append(
            (
                "Global",
                [(port_template_groups.pk, port_template_groups) for port_template_groups in global_port_template_groups],
            )
        )
        ...
        if site is not None:
            # Add non-grouped site VLANs
            site_port_template_groups = PortTemplateGroup.objects.filter(site=site)
            port_template_groups_choices.append(
                (
                    site.name,
                    [(port_template_groups.pk, port_template_groups) for port_template_groups in site_port_template_groups],
                )
            )
