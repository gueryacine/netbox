...
class VirtualMachine(ChangeLoggedModel, ConfigContextModel, CustomFieldModel):
    ...
    status = models.PositiveSmallIntegerField(
        choices=VM_STATUS_CHOICES, default=DEVICE_STATUS_ACTIVE, verbose_name="Status"
    )
        ...
        for field in ["primary_ip4", "primary_ip6"]:
            ...
            if ip is not None:
                if ip.interface in interfaces:
                    ...
                    and self.primary_ip4.nat_inside.interface in interfaces
                ):
                    pass
                else:
                    raise ValidationError(
                        {
                            field: "The specified IP address ({}) is not assigned to this VM.".format(
                                ip
                            )
                        }
                    )
