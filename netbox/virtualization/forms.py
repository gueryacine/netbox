...
VIFACE_FF_CHOICES = ((IFACE_FF_VIRTUAL, "Virtual"),)
...
class ClusterTypeCSVForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {"name": "Name of cluster type"}
...
class ClusterGroupCSVForm(forms.ModelForm):
    ...
    class Meta:
        ...
        help_texts = {"name": "Name of cluster group"}
...
class ClusterCSVForm(forms.ModelForm):
    type = forms.ModelChoiceField(
        ...
        error_messages={"invalid_choice": "Invalid cluster type name."},
    )
    group = forms.ModelChoiceField(
        ...
        error_messages={"invalid_choice": "Invalid cluster group name."},
    )
...
class ClusterFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    type = FilterChoiceField(
        ...
        widget=APISelectMultiple(
            api_url="/api/virtualization/cluster-types/", value_field="slug"
        ),
    )
...
class ClusterAddDevicesForm(BootstrapMixin, ChainedFieldsMixin, forms.Form):
    ...
    site = ChainedModelChoiceField(
        ...
        widget=APISelect(
            ...
            filter_for={"rack": "site_id", "devices": "site_id"},
        ),
    )
                    ...
                    ip_choices.append(
                        (
                            ...
                            [
                                (
                                    ip.id,
                                    "{} ({})".format(ip.address, ip.nat_inside.address),
                                )
                                for ip in nat_ips
                            ],
                        )
...
class VirtualMachineFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    region = FilterChoiceField(
        ...
        required=False,
        widget=APISelectMultiple(
            api_url="/api/dcim/regions/", value_field="slug", null_option=True
        ),
    )
    site = FilterChoiceField(
        queryset=Site.objects.all(),
    ...
    status = forms.MultipleChoiceField(
        choices=VM_STATUS_CHOICES, required=False, widget=StaticSelect2Multiple()
    )
