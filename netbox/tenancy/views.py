...
from ipam.models import IPAddress, Prefix, VLAN, VRF, PortTemplate
...
class TenantView(View):
    def get(self, request, slug):
        ...
        stats = {
            ...
            "rackreservation_count": RackReservation.objects.filter(
                tenant=tenant
            ).count(),
            ...
            "port_template_count": PortTemplate.objects.filter(tenant=tenant).count(),
        ...
        return render(
            request, "tenancy/tenant.html", {"tenant": tenant, "stats": stats}
        )
