...
class ProviderView(View):
    def get(self, request, slug):
        ...
        circuits = (
            Circuit.objects.filter(provider=provider)
        ...
        return render(
            ...
            {"provider": provider, "circuits": circuits, "show_graphs": show_graphs},
        )
...
class CircuitView(View):
    def get(self, request, pk):
        ...
        circuit = get_object_or_404(
            Circuit.objects.select_related("provider", "type", "tenant__group"), pk=pk
        )
        termination_a = (
            CircuitTermination.objects.select_related(
                "site__region", "connected_endpoint__device"
            )
            .filter(circuit=circuit, term_side=TERM_SIDE_A)
            .first()
        )
        termination_z = (
            CircuitTermination.objects.select_related(
            ...
            .filter(circuit=circuit, term_side=TERM_SIDE_Z)
            .first()
        )
