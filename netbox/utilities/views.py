...
class ObjectListView(View):
    ...
    def get(self, request):
        ...
        if hasattr(model, "tags"):
            tags = model.tags.annotate(count=Count("taggit_taggeditem_items")).order_by(
                "name"
            )
        else:
            tags = None
...
class ObjectEditView(GetReturnURLMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            msg = "{} {}".format(
                "Created" if obj_created else "Modified", self.model._meta.verbose_name
            )
    ...
    def post(self, request, **kwargs):
        ...
        if request.POST.get("_all"):
            if self.filter is not None:
                pk_list = [
                    obj.pk
                    for obj in self.filter(request.GET, model.objects.only("pk")).qs
                ]
            else:
        ...
        if "_create" in request.POST:
            ...
            if form.is_valid():
                ...
                for obj in data["pk"]:
                    ...
                    for name in names:
                        component_data = {self.parent_field: obj.pk, "name": name}
